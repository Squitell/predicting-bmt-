...
def test_handle_missing_values():
    ...
    # Compute median using the same approach.
    median_A = pd.to_numeric(df.replace("?", np.nan).infer_objects(copy=False)['A'], errors='coerce').median()
    assert df_processed['A'].iloc[1] == median_A
    
    # For column B, fill with mode.
